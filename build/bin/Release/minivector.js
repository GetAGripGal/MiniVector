...
var wasmBinaryFile;
wasmBinaryFile = instance.srcinfo.wasm_binary;
...
var ASM_CONSTS = {
  72352: ($0) => { return instance.c_api.createSpecialWindow($0); },  
 72403: ($0, $1) => { instance.c_api.handle2Wnd($0).setTitle(UTF8ToString($1)) },  
 72464: ($0, $1, $2) => { instance.c_api.handle2Wnd($0).setSize($1, $2) },  
 72514: ($0) => { instance.c_api.showWindow($0) },  
 72548: ($0) => { instance.c_api.handle2Wnd($0).center() },  
 72591: ($0, $1, $2, $3) => { const title = UTF8ToString($0); const message = UTF8ToString($1); const icon = UTF8ToString($2); const dlg = parent.w96.ui.DialogCreator.alert(message, { title, icon }); if($3 === 1) { Asyncify.handleAsync(async()=>{ await dlg.wait(); }); } },  
 72836: () => { instance.c_api.requestTerminal(); },  
 72874: ($0) => { instance.onExit($0); },  
 72899: ($0) => { instance.c_api.setProcessTitle(UTF8ToString($0)); },  
 72953: ($0) => { Asyncify.handleAsync(()=> new Promise((resolve) => { setTimeout(()=>resolve(), $0); })); }
};
...
// end include: preamble.js
  ...
  allocateUnusedWorker() {
        ...
        var pthreadMainJs = locateFile('minivector.worker.js');
  ...
  if (ENVIRONMENT_IS_PTHREAD)
    return proxyToMainThread(9, 0, 1, title);
  ...
  onMouseButtonChanged:(event, status) => {
        ...
        if (GLFW.active.mouseButtonFunc) {
          ((a1, a2, a3, a4) => { throw 'Internal Error! Attempted to invoke wasm function pointer with signature "viiii", but no such functions have gotten exported!' })(GLFW.active.id, eventButton, status, GLFW.getModBits(GLFW.active));
        }
      },
