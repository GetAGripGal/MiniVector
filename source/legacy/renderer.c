...
{
    mv_legacy_renderer *renderer = malloc(sizeof(mv_legacy_renderer));
...
void mv_legacy_renderer_draw(mv_legacy_renderer *renderer, mv_legacy_display *display, mv_color_t primary, mv_color_t secondary)
{
    ...
    glBufferData(GL_ARRAY_BUFFER, sizeof(mv_point_t) * display->point_count, display->points, GL_DYNAMIC_DRAW);
...
static void create_buffers(mv_legacy_renderer *renderer)
{
    ...
    glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, sizeof(mv_point_t), (void *)0);
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    glBindVertexArray(0);
